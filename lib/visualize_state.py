...
def plot_history(log_data: dict[str, np.ndarray],
    ...
    plt.figure()
    ax = plt.gca()
    plot_environment(ax, x_lim, y_lim)

    interval = 500
    x_car, y_car, psi_car = log_data['car'][::interval, 0], log_data['car'][::interval, 1], log_data['car'][::interval,
                                                                                            2]
    ...
    plt.title(f"The position of the car and trailer (dt = {dt})")
...
def plot_live(log_data: dict[str, np.ndarray],
    ...
    ax.legend(loc='upper center', handles=handles + [yellow_patch, purple_patch, grey_patch],
